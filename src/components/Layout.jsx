      ...
      <div className="px-5 py-6">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/30 flex-shrink-0">
              <Sparkles size={20} className="text-white" />
            </div>
            <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-amber-400 to-orange-600 blur-lg opacity-30 animate-pulse-soft" />
          </div>
      ...
      <nav className="flex-1 px-3 py-5 space-y-1" aria-label="Navegación principal">
        {NAV_ITEMS.map(({ id, label, Icon }) => {
          ...
          return (
            ...
            >
              {active && (
                <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-amber-500/[0.12] to-orange-500/[0.06] border border-amber-500/20" />
              )}
              {active && (
              ...
              <Icon className={`relative z-10 flex-shrink-0 transition-all duration-200 group-hover:scale-110 ${active ? 'text-amber-400' : ''}`} size={18} />
      ...
      <div className="px-4 pb-5 pt-4">
        <div className="p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
          ...
          <div className="h-1.5 bg-white/[0.05] rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-amber-500 via-orange-500 to-amber-400 rounded-full transition-all duration-1000 shadow-[0_0_8px_rgba(245,158,11,0.4)]"
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#06080f]">
      ...
      <aside className="hidden lg:flex w-[260px] bg-[#0a0d14]/90 backdrop-blur-2xl border-r border-white/[0.04] flex-col flex-shrink-0 relative">
        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-amber-500/[0.04] to-transparent pointer-events-none" />
        <SidebarContent />
      </aside>
